...
class LSMDC_DataLoader(Dataset):
    ...
    def __init__(
        ...
        if self.need_object:
            ...
            if os.path.isfile(obj_path):
                ...
                vid_path_list = [(vid[0], os.path.join(object_path, self.subset, 'inference', 'vinvl_vg_x152c4','tsv_files', f"{vid[0]}.json")) for vid in self.iter2video_pairs_dict.values()]
    ...
    def __getitem__(self, feature_idx):
        ...
        if self.need_object:
            object_tensor = self.object_bbox_dict[clip_id]['objects']
            attr_sentence = self.object_bbox_dict[clip_id]['attr_sentence']
            object_cls_tensor = self.object_bbox_dict[clip_id]['obj_cls']
            attr_pairs_text, attr_pairs_mask, attr_pairs_segment, choice_video_ids = self._get_text(clip_id, attr_sentence)
            
            conf_set = self.object_bbox_dict[clip_id]['conf_set']

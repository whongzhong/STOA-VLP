...
class MSVD_QA_DataLoader(Dataset):
    """MSVD dataset loader."""
    def __init__(
            self,
            json_path,
        ...
        with open(json_path, 'r') as f:
            ...
            for item in data:
                target_dict = {
                    'video_id': vid_mapping[str(item['video_id'])],
                    'id_mapping': item['video_id'],
        ...
        if self.need_object:
            ...
            obj_path = os.path.join(object_path, f"object_attr_msvd_qa_{self.subset}_{object_count}.pth")
            if os.path.isfile(obj_path):
                print(f"load from {obj_path}")
                self.object_bbox_dict = torch.load(obj_path)
            else:
                vid_path_list = [(vid_mapping[str(item['video_id'])], os.path.join(object_path, 'tsv_files', f"{vid_mapping[str(item['video_id'])]}.json")) for item in data]
    ...
    def __getitem__(self, idx):
        ...
        id_mapping = self.sentences_dict[idx]['id_mapping']
        if self.need_object:
        ...
        if self.need_object:
            return question_text, question_mask, pairs_segment, video, video_mask, answer_ids, align_text, align_mask, object_tensor, attr_pairs_text, attr_pairs_mask, attr_pairs_segment, object_cls_tensor, id_list, id_mapping
        else:
            return question_text, question_mask, pairs_segment, video, video_mask, answer_ids, id_mapping

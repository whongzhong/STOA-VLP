...
class CLIP4Clip(CLIP4ClipPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, video, video_mask=None):
        ...
        sequence_output, visual_output, text_tokens = self.get_sequence_visual_output(input_ids, token_type_ids, None ,video, video_mask, shaped=True, video_frame=video_frame)
    ...
    def get_mc_metrics(self, options, options_mask, options_segment, video, video_mask, answer):
        ...
        for i in range(options.shape[1]):
            input_ids = options[:,i,:].view(-1,options.shape[-1])
            token_type_ids = options_segment[:,i,:].view(-1,options.shape[-1])
            attention_mask = options_mask[:,i,:].view(-1,options.shape[-1])

            sequence_output,_ = self.get_sequence_output(input_ids, token_type_ids, None, shaped=True)
            if i == 0:
                options_output = sequence_output
            else:
                options_output = torch.cat((options_output,sequence_output),1)
        
        options_output = options_output / options_output.norm(dim=-1, keepdim=True)
        visual_output = visual_output / visual_output.norm(dim=-1, keepdim=True)
        predicted_answer =[]
        for bz in range(visual_output.shape[0]):
            options_score = options_output[bz].matmul(visual_output[bz,:].t()).squeeze()
            predicted_answer.append(options_score.max(dim=-1)[1])
            

        predicted_answer = torch.tensor(predicted_answer).to(answer.device)

        n_correct = (predicted_answer == answer).sum().item()
        n_sample = visual_output.shape[0]
        return n_correct , n_sample , 1
    ...
    def get_visual_output(self, video, video_mask, shaped=False, video_frame=-1):
        ...
        visual_hidden, _ = self.clip.encode_image(video, video_frame=video_frame)

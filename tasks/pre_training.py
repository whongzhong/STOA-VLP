...
def get_args(description='CLIP4Clip on Retrieval Task'):
    ...
    parser.add_argument("--use_sim_loss", action='store_true', help="Whether to incorparate object infomation")
...
def init_model(args, device, n_gpu, local_rank):
    ...
    model = CLIP4Clip.from_pretrained(args.cross_model, cache_dir=cache_dir, state_dict=model_state_dict, task_config=args, object_count=args.object_count, restore_path=restore_path)
...
def save_model(epoch, args, model, sampler=None, type_name="", optimizer=None, step=None):
        ...
        for bid, batch in tqdm(enumerate(test_dataloader), total=len(test_dataloader)):
            ...
            if multi_sentence_:
                ...
                if args.phrase_fuse == 'fusion':
                    ...
                    attr_text_tokens = attr_text_tokens + model.clip.object_text_fusion_positional_embedding[:max_text_len, :]
...
def train_epoch(epoch, args, model, train_dataloader, test_dataloader, device, n_gpu, optimizer, scheduler, global_step, sampler, local_rank=0, val_loader=None, tokenizer=None):
    global logger
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        object_tensor = None
        if args.need_object:
            input_ids, input_mask, segment_ids, video, video_mask,fusion_labels, object_tensor, attr_pairs_text, attr_pairs_mask, attr_pairs_segment, object_cls_tensor, id_list, obj_vocab_idx, prd_id_list, verb_pos_tag, noun_pos_tag  = batch
            vtm_loss, mlm_loss, plm_loss, matching_loss, sim_loss = model(input_ids, segment_ids, input_mask, video, video_mask,fusion_labels, objects=object_tensor, object_layer=object_layer, attr_triple = (attr_pairs_text, attr_pairs_mask, attr_pairs_segment), attr_sim=args.attr_sim, insert_attr=args.insert_attr, replace_cls=args.replace_cls, phrase_fuse=args.phrase_fuse, object_fuse=args.object_fuse, obj_cls_tuple=(object_cls_tensor, id_list, obj_vocab_idx, prd_id_list), pos_tag=(verb_pos_tag, noun_pos_tag), tokenizer=tokenizer, use_action=args.use_action, use_sim_loss = args.use_sim_loss, action_only=args.action_only)
        else:
        ...
        if epoch == 0:
            loss = vtm_loss + mlm_loss + plm_loss + matching_loss + sim_loss
        else:
            loss = vtm_loss + mlm_loss + plm_loss + matching_loss
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            ...
            if global_step % 1000 ==0 and args.rank == 0:
                output_model_file = save_model(epoch, args, model, sampler=sampler, type_name="", optimizer=optimizer, step=global_step)
            if global_step % log_step == 0 and local_rank == 0:
                logger.info(f'step is {step}, vtm_loss is {float(vtm_loss)},mlm_loss is {float(mlm_loss)}, plm_loss is {float(plm_loss)}, sim_loss is {float(sim_loss)}, matching_loss is {float(matching_loss)}')
    ...
    if args.do_train:
        ...
        for epoch in range(restore_dict['epoch'], args.epochs):
            tr_loss, global_step = train_epoch(epoch, args, model, train_dataloader, test_dataloader,device, n_gpu, optimizer,
                                               scheduler, global_step, train_sampler, local_rank=args.local_rank, val_loader=val_dataloader, tokenizer=tokenizer)
            # break
            if args.local_rank == 0:
